<template>
    <div class="space-y-4">
        <SCTableFilter v-model="search" :loading="loading" :initial-search="search" @update:model-value="handleSearch" :filters="filters">
            <template #filters="{ filters }">
                <SCTableFacetedFilter v-for="filter in filters" :column="filter.column" :title="filter.label" :options="filter.options || []" />
            </template>
            <template #actions>
                <div class="flex space-x-2">
                    <SCTableImport
                        v-if="config?.import.endpoint"
                        :import="config.import"
                        @import-complete="handleImportComplete"
                        @import-error="handleImportError"
                    />
                    <SCTableExport
                        v-if="config?.export?.endpoint"
        ...
        <Table>
            ...
            <TableBody>
                ...
                <TableRow v-if="!data.length">
                    <TableCell :colspan="columns.length + (hasActions ? 1 : 0) + (config?.selectable ? 1 : 0)" class="text-center">
                        No results found.
                    </TableCell>
                </TableRow>
            </TableBody>
        ...
        <div v-if="pagination" class="flex items-center justify-end space-x-2">
            <SCTablePagination :pagination="pagination" @page-change="handlePageChange" @per-page-change="handlePerPageChange" />
        </div>

        <!-- BulkActions -->
        <SCTableBulkActions
            v-if="config?.selectable && bulkActions && selectedItems.length > 0"
            :selected-items="selectedItems"
            :actions="bulkActions"
            :total-records="pagination?.total || 0"
            :selecting-all="selectingAllRecords"
...
import SCTableExport from './SCTableExport.vue';
...
import SCTableImport from './SCTableImport.vue';
...
const updateTableState = (newParams: Record<string, any>) => {
    const query = Object.fromEntries(Object.entries(newParams).filter(([_, value]) => value !== null && value !== undefined && value !== ''));
...
// Handlers for import/export events
